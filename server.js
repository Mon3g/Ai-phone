...
function decodeMulawToPcm(ulawBuffer) {
  ...
  for (let i = 0; i < ulawBuffer.length; i++) {
    const sample = MULAW_DECODE_TABLE[ulawBuffer[i]];
  ...
  const interval = setInterval(() => {
    if (audioBuffer.length > 0) {
      ...
      openaiWS.send(JSON.stringify({
        ...
        response: { modalities: ["audio"], instructions: "Reply conversationally and briefly.", voice: "verse" }
      }));
    }
  }, 1000);
...
async function connectToOpenAI() {
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.on("open", () => { console.log("🔗 Connected to OpenAI Realtime API"); resolve(ws); });
    ws.on("error", (err) => { console.error("❌ OpenAI connection error:", err); reject(err); });
  });
}
